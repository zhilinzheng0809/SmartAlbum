...
static CGFloat const SAAutoAnalysisImageMaxPixel = 960.0;
static CGFloat const SAAutoAnalysisJPEGQuality = 0.68;
...
- (BOOL)shouldPresentAutoAnalysisPrompt {
    ...
    if (self.tagStore == nil || self.visionService == nil || ![self.visionService isConfigured]) {
        return NO;
    }
...
- (void)startNextAnalysisIfNeeded {
    ...
    if (self.imageManager == nil || self.tagStore == nil || self.visionService == nil || ![self.visionService isConfigured]) {
        self.statusText = [NSString stringWithFormat:@"自动分析未启动，请先完成 %@ 配置。",
...
/**
 * @brief 按顺序逐张准备一批图片请求项，避免同时解码过多大图导致内存峰值过高。
 * @param assets 待读取照片数组。
 * @param index 当前处理下标。
 ...
 */
- (void)handlePreparedBatchItems:(NSArray<SAVisionAnalyzeItem *> *)items
 ...
 * @param error 错误对象。
 */
- (void)finishBatchItems:(NSArray<SAVisionAnalyzeItem *> *)items
    ...
    [self.imageManager requestImageDataAndOrientationForAsset:asset
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
            @autoreleasepool {
                ...
                NSData *compressedData = [self compressedJPEGDataFromImage:image maxPixel:SAAutoAnalysisImageMaxPixel];
...
- (NSData * _Nullable)compressedJPEGDataFromImage:(UIImage *)image maxPixel:(CGFloat)maxPixel {
    ...
    if (maxSide <= maxPixel) {
        return UIImageJPEGRepresentation(image, SAAutoAnalysisJPEGQuality);
    }
    ...
    return UIImageJPEGRepresentation(resized, SAAutoAnalysisJPEGQuality);
}
